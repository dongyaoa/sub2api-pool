<template>
  <article class="card supplier-card grid min-w-0 overflow-hidden 2xl:grid-cols-[232px_minmax(0,1fr)]">
    ...
    <div class="min-w-0">
      <div class="flex flex-wrap items-center justify-between gap-2 border-b border-gray-100 px-4 py-2.5 sm:px-5 dark:border-dark-700">
        <div class="flex items-center gap-3"><span class="text-xs font-medium text-gray-600 dark:text-gray-300">{{ t('upstreamCenter.groupCount', { count: supplier.targets.length }) }}</span><div class="hidden items-center gap-2.5 text-[10px] text-gray-400 sm:flex"><span v-for="status in ['operational', 'degraded', 'error']" :key="status" class="inline-flex items-center gap-1"><i class="h-2 w-1 rounded-full" :class="status === 'operational' ? 'bg-emerald-500' : status === 'degraded' ? 'bg-amber-400' : 'bg-rose-500'"></i>{{ t(`upstreamCenter.status.${status}`) }}</span></div></div>
        <div class="flex items-center gap-2">
          <button v-if="supplier.targets.length > 1" type="button" class="inline-flex items-center gap-1 rounded-md px-1.5 py-1 text-[11px] text-gray-500 transition-colors hover:bg-gray-100 hover:text-gray-700 dark:text-dark-400 dark:hover:bg-dark-700 dark:hover:text-dark-200" @click="emit('order-groups', supplier)"><Icon name="menu" size="xs" />{{ t('upstreamCenter.order.groups') }}</button>
