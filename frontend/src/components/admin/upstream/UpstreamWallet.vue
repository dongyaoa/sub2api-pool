  ...
  <div class="rounded-xl bg-gray-50 p-3.5 dark:bg-dark-900/60">
    <template v-if="wallets.length">
      <div v-for="(wallet, index) in wallets" :key="`${wallet.target_id}-${wallet.wallet_ref}`" :class="index && 'mt-3 border-t border-gray-200/60 pt-3 dark:border-dark-700'">
        <div class="flex items-center justify-between gap-3"><div class="min-w-0"><p class="flex flex-wrap items-center gap-1.5 text-[11px] text-gray-500 dark:text-dark-400">{{ t(wallet.kind === 'key_quota' ? 'upstreamCenter.wallet.quota' : wallet.kind === 'subscription' ? 'upstreamCenter.wallet.subscription' : 'upstreamCenter.wallet.title') }}<span v-if="isNewapiBilling(wallet.billing)" class="rounded border border-gray-200 px-1 text-[9px] dark:border-dark-600">New API</span><span v-if="wallets.length > 1">· {{ wallet.wallet_ref }}</span></p><strong class="mt-1 block text-xl font-semibold tabular-nums tracking-tight" :class="isLowBalance(wallet) ? 'text-rose-600 dark:text-rose-400' : 'text-gray-900 dark:text-gray-100'" data-testid="wallet-balance">{{ wallet.kind === 'key_quota' && wallet.unlimited_quota ? t('upstreamCenter.newapi.unlimited') : money(walletAmount(wallet), wallet.currency) }}</strong></div><button v-if="syncable" type="button" class="rounded-lg p-2 text-gray-400 hover:bg-white hover:text-primary-600 disabled:opacity-50 dark:hover:bg-dark-800" :disabled="busy" :aria-label="t('upstreamCenter.syncBalance')" :title="t('upstreamCenter.syncBalance')" @click="emit('sync', wallet.target_id)"><Icon name="refresh" size="sm" :class="busy && 'animate-spin'" /></button></div>
        <p v-if="isNewapiBilling(wallet.billing) && wallet.kind === 'wallet' && (wallet.unlimited_quota || wallet.quota_remaining != null)" class="mt-1.5 text-[11px] text-gray-500 dark:text-dark-400" data-testid="wallet-key-quota">{{ t('upstreamCenter.wallet.quota') }} · {{ wallet.unlimited_quota ? t('upstreamCenter.newapi.unlimited') : money(wallet.quota_remaining, wallet.currency) }}</p>
        <p v-if="wallet.status !== 'ok'" class="mt-1.5 text-[11px] leading-4" :class="wallet.status === 'error' ? 'text-amber-600 dark:text-amber-400' : 'text-gray-400 dark:text-dark-400'" :title="upstreamSyncError(wallet.error, t)">{{ t(wallet.status === 'unsupported' ? 'upstreamCenter.wallet.unsupported' : wallet.status === 'error' ? 'upstreamCenter.wallet.error' : 'upstreamCenter.wallet.pending') }}</p>
        <p v-if="wallet.error" class="mt-1 break-words text-[11px] leading-4 text-amber-600/90 dark:text-amber-400">{{ upstreamSyncError(wallet.error, t) }}</p>
        <p v-if="isNewapiBilling(wallet.billing) && wallet.billing?.error && wallet.billing.error !== wallet.error" class="mt-1.5 text-[11px] leading-5" :class="wallet.billing.error.includes('newapi_account_auth_required') ? 'text-amber-700 dark:text-amber-400' : 'text-gray-500 dark:text-dark-400'" data-testid="newapi-wallet-notice">{{ upstreamSyncError(wallet.billing.error, t) }}</p>
    ...
    <div v-if="showKeyUsage" class="mt-3 border-t border-gray-200/60 pt-3 dark:border-dark-700" data-testid="key-upstream-usage">
      ...
      <p v-if="isNewapiBilling(keyWallet?.billing)" class="mt-1 text-[10px] leading-5 text-gray-400 dark:text-dark-400">{{ t(keyWallet?.currency === 'QUOTA' ? 'upstreamCenter.newapi.rawUsageHint' : 'upstreamCenter.newapi.usageHint') }}</p>
...
const isLowBalance = (wallet: UpstreamBalanceSnapshot) => {
  if (wallet.currency === 'QUOTA' || (wallet.kind === 'key_quota' && wallet.unlimited_quota)) return false
