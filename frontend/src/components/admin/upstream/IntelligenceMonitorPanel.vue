...
const visiblePlans=computed(()=>scopePlans.value.filter(plan=>(!sourceFilter.value||plan.source_type===sourceFilter.value)&&[plan.name,plan.source_name,plan.supplier_note,plan.group_note,plan.notes].join(' ').toLowerCase().includes(search.value.trim().toLowerCase())))
...
onMounted(()=>{void load();timer=setInterval(()=>{if(!document.hidden&&!loading.value&&!orderDialog.value)void load()},5000)})
