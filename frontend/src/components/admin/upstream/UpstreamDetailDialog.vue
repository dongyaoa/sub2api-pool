  ...
  <BaseDialog :show="show" :title="target?.name || supplier?.name || t('upstreamCenter.details')" width="extra-wide" @close="emit('close')">
    <template v-if="show">
      ...
      <div v-else class="space-y-5">
        <div class="flex flex-wrap items-end justify-between gap-3"><div class="min-w-0 flex-1 basis-40"><label for="history-model" class="input-label">{{ t('upstreamCenter.model') }}</label><Select id="history-model" v-model="selectedModel" :options="modelOptions" :searchable="false" :aria-label="t('upstreamCenter.model')" /></div><div class="min-w-[120px]"><label for="history-window" class="input-label">{{ t('upstreamCenter.range') }}</label><Select id="history-window" v-model="historyWindow" :options="windowOptions" :searchable="false" :aria-label="t('upstreamCenter.range')" /></div><button type="button" class="btn btn-secondary" :disabled="loading" @click="loadHistory"><Icon name="refresh" size="sm" :class="loading && 'animate-spin'" /><span class="ml-2">{{ t('upstreamCenter.refresh') }}</span></button></div>
...
const windowOptions = computed(() => windows.map(value => ({ value, label: t(`upstreamCenter.ranges.${value}`) })))
