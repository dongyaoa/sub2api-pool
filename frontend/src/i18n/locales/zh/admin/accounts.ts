export default {
    accounts: {
      ...
      proxyPool: {
        title: '代理池（可选）',
        hint: '按各代理并发配额平滑轮询，自动跳过不可用线路；账号总并发为各代理并发之和。',
        add: '添加代理',
        remove: '移除代理',
        proxy: '代理',
        selectProxy: '请选择代理',
        ...
        availableCount: '还可添加 {count} 个',
        defaultConcurrency: '新增代理并发',
        defaultConcurrencyHint: '新增代理默认 20 并发，可分别调整。',
        limitsHint: '最多可绑定 {maxEntries} 个代理，总并发不可超过 {maxConcurrency}。',
        addAll: '添加全部可用（{count}）',
        allAdded: '暂无可添加的代理，请检查代理状态或添加新代理。',
        unavailable: '此代理当前不可用，请更换或移除。',
        missingProxy: '代理 #{id}（已不可用）'
      },
      concurrency: '并发数',
      loadFactor: '负载因子',
      ...
      testModelsLoadFailed: '测试模型加载失败，请关闭后重试。',
      testNoModelsAvailable: '当前账号没有可用的测试模型。',
      testProxy: '本次测试代理',
